...
class App extends Component {
  ...
  componentDidMount(){
   ...
   fetch(url, {
     ...
     .then((res) => res.json())
     .then((data) =>this.setState({ picturesDB: data.items },function(){
        this.getAllSitesName()
         } ))
 }
...
//    let url = `https://checkfrontcom.checkfront.com/api/3.0/item?istart_date=${selectedDay}&end_date=${selectedDay}`
//    fetch(url, {
//      method: "GET", // *GET, POST, PUT, DELETE, etc.
//      mode: "cors", // no-cors, *cors, same-origin
//      cache: "no-cache", // *default, no-cache, reload, force-cache, only-if-cached
...
getAvailabeSites(){
   ...
   for (let item_id in sitesAvailablity){
      if ( !repeated_sites_to_ignore.includes(item_id) ){
         if(sitesAvailablity[item_id][date] === 1 && item_id in this.state.sites_by_id ) {
            if( sitesData[item_id]["product_group_children"].length > 0){
               ...
               repeated_sites_to_ignore = repeated_sites_to_ignore.concat(to_ignore.reduce( (acc,curr) => acc.concat([curr["item_id"]]),[]))
            }
            avilableSites[item_id] = {
               name:sitesData[item_id]["name"],
               url:this.state.sites_by_id[item_id]?.["url"],
               region:this.convertRegionID2Name(sitesData[item_id]["category_id"])
            }
         }
...
handleChange = (event) => {
  ...
  render() {
    ...
    const fortnightAway = new Date(Date.now() + 12096e5); // set a calnder disabler for 2 weeks away
         ...
         if (this.state[region]){
            ...
            sitesTable[region].push (
              {  name:name,
                 element: 
                  <a href={url}>
                        <div className="card w3-blue" key={id} 
                        style={{  backgroundImage:"url("+background+")"  }} >
                        <a className ="hyperLink" >{name} </a>
                        </div>
                     </a>
                  }
   ...
   const fuseNorth = new Fuse(sitesTable["north"], {keys: ['name'], threshold:0.5})
   const fuseCenter = new Fuse(sitesTable["center"], {keys: ['name'], threshold:0.5})
   const fuseJerusalem = new Fuse(sitesTable["Jerusalem"], {keys: ['name'], threshold:0.5})
   const fuseSouth = new Fuse(sitesTable["south"], {keys: ['name'], threshold:0.5})
    ...
    return (
      ...
      <div className="App" >
         ...
         <Container fluid className="text-light bg-teva-green py-3">
           <Row>
              <Col>
              <h1>לאן נטייל מחר? 🏕️ <a class="text-light" href="http://ganleumi.online">ganleumi.online</a></h1>
                                 ...
                                 this.state.formatDate.length > 0?
                                    <div>
                                       ...
                                       <Row>
                                          <Col>
                                                   חיפוש:
                                                <input type="text" value={this.state.input} onChange={this.handleChange} placeholder="הכנס שם פארק לאומי"  />
                                          </Col>
                                       </Row>
                                    </div>
                                 : <div></div>
                                    ...
                                    
                                    {
                                       ...
                                       <div>
                                          ...
                                          <section className="basic-grid">
                                             {
                                                input.length=== 0 ?  sitesTable.north.map(item => item.element) :
                                                fuseNorth.search(this.state.input).map(x => x.item.element)
                                             }
                                          </section>
                                          ...
                                          <section className="basic-grid">
                                             {
                                                input.length=== 0 ?  sitesTable.center.map(item => item.element) :
                                                fuseCenter.search(this.state.input).map(x => x.item.element)
                                             }                                          </section>
                                          ...
                                          <section className="basic-grid">
                                            {
                                                input.length=== 0 ?  sitesTable.Jerusalem.map(item => item.element) :
                                                fuseJerusalem.search(this.state.input).map(x => x.item.element)
                                             }                                      
                                          </section>
                                          ...
                                          <section className="basic-grid">
                                             {
                                                input.length=== 0 ?  sitesTable.south.map(item => item.element) :
                                                fuseSouth.search(this.state.input).map(x => x.item.element)
                                             }                                            
                                          </section>
                                       </div>
            ...
            <Container>
               <Row>
                  <Col>
                     ...
                     <div className="my-2">
                        <span>🚮 טיול נעים! ובבקשה אל תשאירו אשפה בטבע </span>
                     </div>
                  </Col>
               </Row>
            </Container>
